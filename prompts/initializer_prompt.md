...
- 当前模式：`{init_mode}`
- 若模式为 `scaffold-only`：只允许完成初始化脚手架，不允许开始任何功能实现。
- 若模式为 `open`：允许在初始化完成后继续实现首个功能，但仍需遵守下文日志与提交约束。
...
- 初始化阶段禁止将 `events.jsonl` 与 `run_logs.jsonl` 纳入 Git 跟踪或提交。
- 初始化阶段禁止读取 `events.jsonl` 作为上下文输入。
- 若仓库中已存在上述文件的跟踪记录，需先取消跟踪（保留本地文件），再继续。
...
- `scaffold-only`：不允许开始实现，完成初始化后立即结束会话。
- `open`：允许开始实现 feature_list.json 中最高优先级功能，且一次只处理一个功能。
...
4. 将环境留在干净、工作的状态
5. 若模式为 `scaffold-only`，完成第 1-4 步后立即结束，不进入功能开发
...
5. `events.jsonl` 与 `run_logs.jsonl` 没有被提交
